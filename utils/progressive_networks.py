...
class TrivialUpBlock(nn.Module):
    def __init__(self, in_channels, out_channels, activation, normalize=False):
        super().__init__()
    ...
    def forward(self, feature_map):
        if self.normalize:
            return self.thinner(self.activation(F.normalize(self.upconv(feature_map))))
        return self.thinner(self.activation(self.upconv(feature_map)))
...
class TrivialDownBlock(nn.Module):
    ...
    def forward(self, feature_map):
        if self.normalize:
            return self.conv(self.activation(F.normalize(self.fattener(feature_map))))
...
class TrivialGeneratorLight(nn.Module):
    def __init__(self):
        ...
        self.up1 = TrivialUpBlock(128, 112, self.activation, True)  # 4x4 -> 8x8
        self.up2 = TrivialUpBlock(112, 96, self.activation, True)  # 8x8 -> 16x16
        self.up3 = TrivialUpBlock(96, 80, self.activation, True)  # 16x16 -> 32x32
        self.up4 = TrivialUpBlock(80, 64, self.activation, True)  # 32x32 -> 64x64
        self.up5 = TrivialUpBlock(64, 32, self.activation, True)  # 64x64 -> 128x128
        self.up6 = TrivialUpBlock(32, 16, self.activation)  # 128x128 -> 256x256
    ...
    def forward(self, z, levels=6):
        ...
        for i in range(levels):
            img = self.activation(self.blocks[i](img))
        return img
...
class TrivialDiscriminatorLight(nn.Module):
    def __init__(self):
        ...
        self.down1 = TrivialDownBlock(16, 32, self.activation)  # 256x256 -> 128x128
        self.down2 = TrivialDownBlock(32, 64, self.activation, True)  # 128x128 -> 64x64
        self.down3 = TrivialDownBlock(64, 80, self.activation, True)  # 64x64 -> 32x32
        self.down4 = TrivialDownBlock(80, 96, self.activation, True)  # 32x32 -> 16x16
        self.down5 = TrivialDownBlock(96, 112, self.activation, True)  # 16x16 -> 8x8
        self.down6 = TrivialDownBlock(112, 127, self.activation, True)  # 8x8 -> 4x4
